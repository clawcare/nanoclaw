...
// Mock telegram-html — pass through input unchanged so tests can assert exact content
vi.mock('../telegram-html.js', () => ({
  toTelegramHtml: vi.fn((input: string) => input),
}));
...
describe('TelegramChannel', () => {
  ...
  describe('non-text messages', () => {
    it('downloads photo and includes container path', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /\[Photo\]\nFile saved to: \/workspace\/group\/downloads\/\d+-photo\.jpg/,
          ),
        }),
      );
    });
    ...
    it('downloads photo with caption', async () => {
      ...
      const ctx = createMediaCtx({
        ...
        extra: {
          photo: [{ file_id: 'pic', width: 400, height: 300 }],
        },
      });
      await triggerMediaMessage('message:photo', ctx);
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /\[Photo\]\nFile saved to: \/workspace\/group\/downloads\/\d+-photo\.jpg Look at this/,
          ),
        }),
      );
    });
    ...
    it('falls back to placeholder for photo with no sizes', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('downloads document and includes container path', async () => {
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /\[Document: report\.pdf\]\nFile saved to: \/workspace\/group\/downloads\/\d+-report\.pdf/,
          ),
        }),
      );
    });
    ...
    it('uses fallback name when filename missing', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          content: expect.stringMatching(
            /\[Document: file\]\nFile saved to: \/workspace\/group\/downloads\/\d+-file/,
          ),
        }),
      );
    });
    ...
    it('sanitizes filenames with path separators', async () => {
      ...
      const ctx = createMediaCtx({
        extra: { document: { file_name: '../../etc/passwd' } },
      });
      await triggerMediaMessage('message:document', ctx);

      // ../../etc/passwd → replace / with _ → .._.._etc_passwd → collapse .. → ._._etc_passwd
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({
          content: expect.stringMatching(
            /File saved to: \/workspace\/group\/downloads\/\d+-\._\._etc_passwd/,
          ),
        }),
      );
    });
  ...
  describe('sendMessage', () => {
    it('sends message via bot API with HTML', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();
    ...
    it('falls back to plain text when HTML send fails', async () => {
      ...
      currentBot()
        .api.sendMessage.mockRejectedValueOnce(new Error('Bad Request: can\'t parse'))
        .mockResolvedValueOnce(undefined);

      await channel.sendMessage('tg:100200300', 'has *unbalanced markdown');

      expect(currentBot().api.sendMessage).toHaveBeenCalledTimes(2);
      expect(currentBot().api.sendMessage).toHaveBeenNthCalledWith(
        1,
        '100200300',
        'has *unbalanced markdown',
        { parse_mode: 'HTML' },
      );
      expect(currentBot().api.sendMessage).toHaveBeenNthCalledWith(
        2,
    ...
    it('handles send failure gracefully when both attempts fail', async () => {
      const opts = createTestOpts();
      const channel = new TelegramChannel('test-token', opts);
      await channel.connect();

      currentBot()
        .api.sendMessage.mockRejectedValueOnce(new Error('Markdown fail'))
        .mockRejectedValueOnce(new Error('Network error'));

      // Should not throw even if both Markdown and plain text fail
      await expect(
