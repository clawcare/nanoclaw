...
export function storeMessage(msg: NewMessage): void {
  db.prepare(
    `INSERT OR REPLACE INTO messages (id, chat_jid, sender, sender_name, content, timestamp, is_from_me, is_bot_message, topic_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    msg.id,
    msg.chat_jid,
    msg.sender,
...
export function getNewMessages(
  ...
  const sql = `
    SELECT id, chat_jid, sender, sender_name, content, timestamp, is_from_me, topic_id
    FROM messages
